...
class TestText2SpeechKokoro(unittest.TestCase):
    ...
    def test_text2speech_kokoro_generates_audio(self, mock_play: Mock, mock_sd: Mock, mock_kpipeline: Mock) -> None:
...
class TestAudioPlayback(unittest.TestCase):
    ...
    def test_play_audio_safely_uses_custom_device(self, mock_sd: Mock) -> None:
        ...
        with patch.object(mock_sd, "play") as mock_play:
            Text2Speech._play_audio_safely(audio, original_sample_rate=24000, device=custom_device)
    ...
    def test_play_audio_safely_handles_error(self, mock_print: Mock, mock_sd: Mock) -> None:
        ...
        self.assertTrue(any("Error during safe audio playback" in str(call) for call in mock_print.call_args_list))
